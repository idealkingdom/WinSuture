...
if ($Mode -eq "Check") {
    # Scan logic: returns $true if at least one hosts.bak exists on Desktop folders
...
elseif ($Mode -eq "Apply") {
    ...
    if ($hostsBackups.Count -eq 0) {
        Write-Host "[-] No backup folders containing 'hosts.bak' were found on the Desktop." -ForegroundColor Red
